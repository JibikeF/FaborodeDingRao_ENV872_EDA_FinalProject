...
Note: Google R SF cheat sheet 

Simple feature, coordinate pair, coordinate reference system

Lat-long pertains to locations but doesn't work well for calculations for distance etc. Lat-long is an angular unit of measure. So we convert them into planar units using projection. 

UTM - Universal Transverse Mercator: Makes longitudinal slices that conserve accuracy within the given slice.

EPSG numbers - catalog of coordinate systems

sv_transform (I think) - helps us change coordinate systems
...
Note: HUC is pertains to hydrological drainage. an eight digit HUC is a very specific area; fewer digits pertain to a larger region
...
ggplot()+
  ...
  geom_sf(data=HUC8_sf_filtered, fill='darkgreen')
...
#in order to not drop the 0 from the HUC, we need to do some additional massaging. substr helps us choose a part of a string
